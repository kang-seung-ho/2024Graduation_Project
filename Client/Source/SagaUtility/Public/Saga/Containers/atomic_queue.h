...
namespace atomic_queue
{
	...
	namespace details
	{
		...
		template<class T>
		[[nodiscard]]
		static constexpr T decrement(T x) noexcept
		{
			return x - 1;
		}
		...
		template<class T>
		[[nodiscard]]
		static constexpr T increment(T x) noexcept
		{
			return x + 1;
		}
		...
		template<class T>
		[[nodiscard]]
		static constexpr T or_equal(T x, unsigned u) noexcept
		{
			return x | x >> u;
		}
		...
		[[nodiscard]]
		static constexpr T or_equal(T x, unsigned u, Args... rest) noexcept
		{
			return or_equal(or_equal(x, u), rest...);
		}

		[[nodiscard]]
		static constexpr uint32_t round_up_to_power_of_2(uint32_t a) noexcept
		{
			return increment(or_equal(decrement(a), 1, 2, 4, 8, 16));
		}

		[[nodiscard]]
		static constexpr uint64_t round_up_to_power_of_2(uint64_t a) noexcept
		{
