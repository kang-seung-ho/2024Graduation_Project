...
demo::OnGameLoadedSignal(demo::Framework& framework, iconer::app::User& user)
{
	if (not user.TryChangeState(iconer::app::UserStates::InRoom, iconer::app::UserStates::MakingGame))
	{
		...
		else
		{
			if (auto room = framework.FindRoom(user.myRoomId); room != nullptr)
			{
				...
				if (room->TryCancelReady())
				{
					room->proceedMemberCount.CompareAndSet(cnt, cnt + 1, std::memory_order_release);

					room->isGameReadyFailed.Store(true, std::memory_order_release);

					// cannot prepare the game: The client has another state
	...
	{
		if (not room->HasMember(user.GetID()))
		{
			// rollback
			if (user.myRoomId.CompareAndSet(room->GetID(), -1))
			{
				user.TryChangeState(iconer::app::UserStates::MakingGame, iconer::app::UserStates::Idle);
				user.TryChangeState(iconer::app::UserStates::InRoom, iconer::app::UserStates::Idle);
			}
		...
		else
		{
			...
			if (not framework.Schedule(user.loadingContext, user.GetID()))
			{
				...
				room->proceedMemberCount.Store(cnt + 1, std::memory_order_release);

				// mark room as failed
				room->isGameReadyFailed.CompareAndSet(false, true);
				...
				if (room->GetMembersCount() <= cnt + 1)
				{
					...
					room->proceedMemberCount.CompareAndSet(cnt + 1, 0, std::memory_order_release);
				}
